...
impl From<&OpcodeRecord> for OpcodeStats {
    fn from(record: &OpcodeRecord) -> Self {
        ...
        for (i, v) in record.opcode_record.iter().enumerate() {
            ...
            // overall
            opcode_stats.overall.count = opcode_stats
                .overall
                .count
                .checked_add(v.0)
                .expect("overflow");
            opcode_stats.overall.time = opcode_stats
                .overall
                .time
                .checked_add(v.1)
                .expect("overflow");
...
impl ExtraPrint for OpcodeRecord {
    fn print_addition_count(&self) {
        ...
        println!(
            "call_code additional rdtsc count: {}",
            self.additional_count[1]
        );
        println!(
            "delegate_call additional rdtsc count: {}",
            self.additional_count[2]
        );
        println!(
            "static_call additional rdtsc count: {}",
            self.additional_count[3]
        );
        println!();
    }
